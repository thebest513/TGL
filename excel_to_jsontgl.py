...
new_data = True # Does your T_BATCH_JOB.xlsx need pre_process()?
datain = 'T_BATCH_JOB' # input file
help_needed = False # pre_process() argument
mail_off = 'nomail' # pre_process() argument
logic0 = 'logic0' # pre_process() argument
ip = '10.67.70.16:9081' # eBao ip
...
uniq = '' # unique Control-M folder name and jobname

if new_data:
    from pre_process import pre_process
    datain = pre_process(datain, 'T_BATCH_JOB_DEPEND', 'mapping',\
                         'T_COMPONENT_DEF', help_needed, mail_off, logic0)
...
df_folder['Variables'] = [[{"processDate":'2022-04-28'},
                           {"ip":ip},
...
df_jobebao['Command'] = (f'curl -H \"Content-Length: 250\" '                     
            ...
            f':{back_slash2}\"1900-01-01{back_slash2}\"{squigglyR}\" -v -sS')
